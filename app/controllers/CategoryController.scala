...
class CategoryController @Inject()(@Named("receptionist") receptionist: ActorRef)
  ...
  def getProjectsForCategory(categoryID: String, offset: Int, limit: Int) =  Action.async {
    request => {
      // Ask receptionist to get categories
      receptionist ? RetrieveCategoryProjects(categoryID, offset, limit) map {
        ...
        // timeout exception
        case e: TimeoutException =>
          AskTimeoutError("Retrieve projects of category failed",
            "Ask Timeout Exception on Actor Receptionist", this.getClass.toString).result
      }
    }
  }
